...
//!
//! EBML is the binary format underlying WebM (and Matroska). It uses variable-length
...
pub fn read_vint<R: Read>(reader: &mut R) -> Result<(u64, usize)> {
    ...
    if first_byte[0] == 0 {
        return Err(WebmError::InvalidVint { offset: 0 });
    }

    // Determine length from leading bit pattern
    let length = first_byte[0].leading_zeros() as usize + 1;
    if length > MAX_VINT_LENGTH {
        return Err(WebmError::VintOverflow);
    }

    // Build the value, masking out the length indicator bits
    // For length 8, the mask would be 0, but we need to handle the case where
    // the entire first byte is consumed by the length marker
...
pub fn read_element_id<R: Read>(reader: &mut R) -> Result<(u32, usize)> {
    ...
    let length = first_byte[0].leading_zeros() as usize + 1;
    if length > 4 {
        return Err(WebmError::InvalidElementId { offset: 0 });
    }
...
pub fn read_element_size<R: Read>(reader: &mut R) -> Result<(Option<u64>, usize)> {
    ...
    let unknown_marker = match length {
        ...
        _ => return Err(WebmError::VintOverflow),
    };
...
pub struct EbmlHeader {
    ...
    /// Document type (must be "webm" for WebM files).
    pub doc_type: String,
...
pub fn read_string(data: &[u8]) -> Result<String> {
    ...
    String::from_utf8(data[..end].to_vec())
        .map_err(|e| WebmError::Other(format!("Invalid UTF-8 string: {}", e)))
}
